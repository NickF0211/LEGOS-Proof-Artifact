...
def check_input_red(model_str, multi_entry=False, profiling=False):
    ...
    res = check_red(model, rules, relations, Action_Mapping, Actions, check_proof=True, model_str=model_str,
                    multi_entry=multi_entry, profiling=profiling)
    # reset
    scalar_mask.clear()
    scalar_type.clear()
...
def check_input_conflict(model_str, multi_entry=False, profiling=False):
    model, rules, concerns, purposes, relations, Action_Mapping, Actions = parse_sleec(model_str, read_file=False)
...
def check_input_purpose(model_str, multi_entry=False, profiling=False):
    model, rules, concerns, purposes, relations, Action_Mapping, Actions = parse_sleec(model_str, read_file=False)
...
def check_input_concerns(model_str, profiling=False):
    model, rules, concerns, purposes, relations, Action_Mapping, Actions = parse_sleec(model_str, read_file=False)
    res = check_concerns(model, rules, concerns, relations, Action_Mapping, Actions, model_str=model_str, profiling=profiling)
    # reset
    scalar_mask.clear()
    scalar_type.clear()
    registered_type.clear()
...
if __name__ == "__main__":
    model, rules, concerns, purposes, relations, Action_Mapping, Actions = parse_sleec(
        "../CaseStudies/covidfree/covidfree.sleec", read_file=True)
