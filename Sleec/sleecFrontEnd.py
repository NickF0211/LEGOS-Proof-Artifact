...
def check_concern():
    ...
    result, response, hl = check_input_concerns(cur_text, profiling=Profiling)
...
def check_situational():
    cur_text = aText.get("1.0", 'end-1c')
    result, response, hl = check_situational_conflict(cur_text, profiling=Profiling)
    new_text.delete('1.0', 'end-1c')
    if result:
        cur_index = 0
...
def check_redundancy():
    cur_text = aText.get("1.0", 'end-1c')
    result, response, hl = check_input_red(cur_text, profiling=Profiling)
    new_text.delete('1.0', 'end-1c')
    if result:
        cur_index = 0
        sorted_hl = sorted(hl, key=lambda k: k[0], reverse=True)
...
def check_conflict():
    cur_text = aText.get("1.0", 'end-1c')
    result, response, hl = check_input_conflict(cur_text, profiling=Profiling)
    new_text.delete('1.0', 'end-1c')
    if result:
        cur_index = 0
        sorted_hl = sorted(hl, key=lambda k: k[0], reverse=True)
...
def check_purpose():
    cur_text = aText.get("1.0", 'end-1c')
    result, response, hl = check_input_purpose(cur_text, profiling=Profiling)
    new_text.delete('1.0', 'end-1c')
    if result:
        cur_index = 0
        sorted_hl = sorted(hl, key=lambda k: k[0], reverse=True)
